"""Standalone script to seed categories into MongoDB Atlas."""
import asyncio
import os
...
async def seed():
    ...
    if existing > 0:
        print(f"Categories already exist ({existing} found). Dropping and re-seeding...")
        await db.categories.drop()
        await db.subcategories.drop()
    ...
    for cat in SEED_CATEGORIES:
        ...
        print(f"  + Category: {cat['name']} (id: {result.inserted_id})")
    ...
    for cat_slug, subcats in SEED_SUBCATEGORIES.items():
        ...
        for sub in subcats:
            ...
            print(f"    - Subcategory: {sub['name']} (under {cat_slug})")
    ...
    total_subs = await db.subcategories.count_documents({})
    print(f"\nDone! Seeded {total_cats} categories and {total_subs} subcategories.")
    client.close()
